...
export class FunctionService {
  ...
  save(sFunction: FunctionObject) {
    return new Observable(observable => {
      this.userService.getCurrentUser().subscribe((user: User) => {
        ...
        this.http.get('system/info').subscribe(
          (results: any) => {
            const savingMethod = sFunction.isNew ? 'POST' : 'PUT';
            sFunction.href =
              results.contextPath + '?api/dataStore/functions/' + sFunction.id;
            this._save(_.omit(sFunction, 'isNew'), savingMethod).subscribe(
              () => {
        ...
        (results: any) => {
          ...
          if (results.length > 0) {
            ...
            this.http.get('system/info').subscribe((response: any) => {
              ...
              forkJoin(this.creationOfNewFunctionObseravble()).subscribe(
                (responses: any) => {
  ...
  createCompletenessFunctions() {
    return new Observable(observable => {
      ...
      this.http.get('dataSets.json?paging=false').subscribe(
        (dataSetResults: any) => {
          this.getId(dataSetResults.dataSets.length + 1).subscribe(
            (codeResults: any) => {
              ...
              this.save(completeness).subscribe(
                (res: any) => {
                  observable.next(res);
                  observable.complete();
                },
  ...
  predictor() {
    return new Observable(observable => {
      ...
      this.http.get('dataElements.json?pageSize=10').subscribe(
        (dataElementResults: any) => {
